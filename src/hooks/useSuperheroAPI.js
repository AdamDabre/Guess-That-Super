    ...
    try {
        ...
        const characterMap = characters.reduce((acc, character) => {
            if (
                ...
                console.warn(
                    `Skipping invalid character entry: ${JSON.stringify(
                        character
                    )}`
                );
            }
...
export async function fetchAndAddPublishersToHeroes() {
    try {
        const API_KEY = "126546965df4bf967813ced41ce1e1f2"; // Replace with your actual API key
        if (!API_KEY) {
            throw new Error(
                "API key is missing. Please set VITE_SUPERHERO_API_KEY in your environment."
            );
        ...
        const heroIds = allHeroes.map((hero) => hero.id);
        const heroes = [];

        // Fetch publisher data for each hero
        for (const id of heroIds) {
            const response = await fetch(
                `https://superheroapi.com/api/${API_KEY}/${id}/biography`
            );
            if (!response.ok) {
                throw new Error(
                    `Failed to fetch publisher for hero ID ${id}: ${response.statusText}`
                );
            }
            ...
            if (data.response === "success") {
                ...
                console.warn(
                    `Failed to fetch publisher for hero ID ${id}: ${data.error}`
                );
            }
        }
        ...
        console.log("Updated heroes saved to characters.json");
    } catch (error) {
